...
void insertValue(int array[10][10]){
	int row, column, arrayInsert[100], insertRow, insertColumn, insertLocation;
	int insertIndex, valueToInsert, spaceLocation;
	...
	insertLocation = ((insertRow - 1) * 10) + insertColumn - 1;
	...
	if(insertLocation < 0 ||  insertLocation > 99){
	...
	if(arrayInsert[99] == 0){		
		for(insertIndex = 99; insertIndex >= insertLocation; insertIndex--){
			arrayInsert[insertIndex] = arrayInsert[insertIndex - 1];		
		}
	
		arrayInsert[insertLocation] = valueToInsert;		
	}
	else if(arrayInsert[0] == 0){
		for(insertIndex = 0; insertIndex < insertLocation; insertIndex++){
			arrayInsert[insertIndex] = arrayInsert[insertIndex + 1];
		}
		
		arrayInsert[insertLocation] = valueToInsert;		
	}
	else if(arrayInsert[0] != 0 && arrayInsert[99] != 0){
		for(insertIndex = 0; insertIndex < 100; insertIndex++){
			if(arrayInsert[insertIndex] == 0){
				spaceLocation = insertIndex;
			}
		}
		
		if(spaceLocation == insertLocation){
			arrayInsert[insertLocation]	= valueToInsert;		
		}
		if(spaceLocation < insertLocation){
			for(insertIndex = spaceLocation; insertIndex < insertLocation; insertIndex++){
				arrayInsert[insertIndex] = arrayInsert[insertIndex + 1];
			}
			arrayInsert[insertLocation]	= valueToInsert;
		}
		...
		printf("Array is full, can't insert values\n\n");
	}
...
void deleteValue(int array[10][10]){
	int row, column, arrayDelete[100], deleteRow, deleteColumn, deleteLocation, hole;
	
	printf("Enter the location that you wish to delete:\n");
	printf("row: ");
	scanf("%d", &deleteRow);
	printf("column: ");	
	scanf("%d", &deleteColumn);
	
	if(((deleteRow - 1) * 10) + deleteColumn > 100){
		printf("\nLocation is out of range\n");
	}
	else{
		deleteLocation = ((deleteRow - 1) * 10) + deleteColumn - 1;
	
		for(row = 0; row < 10; row++){
			for(column = 0; column < 10; column++){
				arrayDelete[(row*10) + column] = array[row][column];
			}
		}
	
		for(hole = deleteLocation; hole < 99; hole++){
			arrayDelete[hole] = arrayDelete[hole + 1];
		}
		...
		for(row = 0; row < 10; row++){
			for(column = 0; column < 10; column++){
				array[row][column] = arrayDelete[(row*10) + column];
			}
		}
	}	
}

void insertDelete(int array[10][10]){
